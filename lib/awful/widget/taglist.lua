...
--
-- As demonstrated in the example above, there is a few "shortcuts" to avoid
...
function taglist.new(args, filter, buttons, style, update_function, base_widget)
    ...
    if argstype == "number" or argstype == "screen" or (argstype == "table" and args.index and args == capi.screen[args.index]) then
    ...
    local deprecated_param = {filter, buttons, style, update_function, base_widget}
    local deprecated_names = {"filter", "buttons", "style",
                              "update_function", "layout"}

    for k=1, 5 do
        local v = deprecated_param[k]
        if v then
            gdebug.deprecate("The `awful.widget.taglist()` `"..deprecated_names[k]
                .."` paramater is deprecated, use `args."..deprecated_names[k].."`.",
            {deprecated_in=5})
            args[deprecated_names[k]] = v
        end
    end

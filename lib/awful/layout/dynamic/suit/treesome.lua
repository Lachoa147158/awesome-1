...
local function add(self, widget)
    ...
    local current = 0
    if not best_c then
        for k,c2 in ipairs(self._tag:clients()) do
...
local module = dynamic.register("treesome", function(t) return ctr(t, "right") end)

module.horizontal   = dynamic.register("treesomeh",   function(t) return ctr(t, "treesomeh"  ) end)
module.horizontal   = dynamic.register("treesomev",   function(t) return ctr(t, "treesomev"  ) end)

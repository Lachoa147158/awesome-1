...
-- paramater, as this is computed during arranging
--
-- If "t" is given, "screen" is ignored, if none are given, the mouse screen is
-- used
--
-- @param[opt] t The tag to query
-- @param[opt] screen The screen
-- @return A table with the workarea (x, y, width, height), the screen
--   geometry (x, y, width, height), the clients, the screen and call
--   the "geometries" table with client as keys and geometry as value
function layout.parameters(t, screen)
    if t then
        screen = tag.getscreen(t) or screen
        t = tag.selected(screen)
    end

...
function icon:fit(_, width, height)
    ...
    local optimal  = beautiful.notification_icon_size or dpi(48)

    local w = self._private.image:get_width()
    local h = self._private.image:get_height()

    if strategy == "resize" then
        return math.min(w, optimal), math.min(h, optimal)
    else
...
function icon:draw(_, cr, width, height)
    ...
    if strategy == "center" and aspect > 1 then
...
-- @param string
-- @see beautiful.notification_icon_size
...
local function new()
    ...
    function tb._private.icon_changed_callback()
        self:set_image(self._private.notification.icon)
    end

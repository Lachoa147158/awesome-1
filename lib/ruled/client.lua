...
--
-- @DOC_ruled_client_rules_maximized_EXAMPLE@
--
-- If you want to set mplayer floating at startup, you can add:
--
-- @DOC_ruled_client_rules_floating_EXAMPLE@
--
...
--
-- @DOC_ruled_client_rules_screens_EXAMPLE@
--
...
function rules.high_priority_properties.intrusive(c, value, props)
    if not value then return end
...
local function request_rules()
    ...
    -- Do this from this module instead of `awful.tag.rules` because it forces
    -- the initialization order to be stable. Doing so in each modules would
    -- potentially make the order unstable if another module were to include
    -- `awful.tag` before `awful.rules` gets initialized.
    capi.tag.emit_signal("request::rules")
...
return setmetatable(rules, {
    __newindex = function(_, k, v)
        if k == "rules" then
            gdebug.deprecate(
                "Use awful.rules.append_rules instead awful.rules.rules",
                {deprecated_in=5}
            )

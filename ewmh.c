...
ewmh_process_state_atom(client_t *c, xcb_atom_t state, int set)
{
    ...
    {
        if(set == _NET_WM_STATE_REMOVE) {
            lua_pushboolean(globalconf.L, false);
            luaA_object_emit_signal(globalconf.L, -2, "request::urgent", 1);
        }
        else if(set == _NET_WM_STATE_ADD) {
            lua_pushboolean(globalconf.L, true);
            luaA_object_emit_signal(globalconf.L, -2, "request::urgent", 1);
        }
        else if(set == _NET_WM_STATE_TOGGLE) {
            lua_pushboolean(globalconf.L, !c->urgent);
            luaA_object_emit_signal(globalconf.L, -2, "request::urgent", 1);
        }
    }
